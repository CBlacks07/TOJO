...
const PLPScreen = ({ products, onOpenProduct, onAdd }) => {
  ...
  const gridCols = width <= 480 ? 1 : width <= 768 ? 2 : 3;
  return (
    <main style={{ maxWidth: '100%', margin: '0 auto', padding: width <= 480 ? '16px 12px 80px' : '32px 32px 80px' }}>
      {/* Breadcrumb */}
      ...
      <div style={{ display: 'flex', gap: width <= 768 ? 0 : 32, alignItems: 'flex-start', flexDirection: width <= 768 ? 'column' : 'row' }}>
        {width > 768 && <FilterRail filters={filters} onChange={setFilters}/>}
        <div style={{ flex: 1, display: 'grid', gridTemplateColumns: `repeat(${gridCols}, 1fr)`, gap: 12, width: '100%' }}>
